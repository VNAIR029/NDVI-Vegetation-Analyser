...
def process_directory(directory_path):
    """Process all images in a directory with more robust searching"""
    ...
    # Extended file type support and more flexible searching
    image_extensions = ['.tif', '.tiff', '.png', '.jpg', '.jpeg']

    # Walk through entire directory tree
    for root, dirs, files in os.walk(directory_path):
        st.write(f"Searching in directory: {root}")
        ...
        st.write(f"Found {len(image_files)} potential image files")
        ...
        for img_file in image_files:
            try:
                with st.spinner(f'Processing {os.path.basename(img_file)}...'):
                    ...
                    if error:
                        st.warning(f"Skipping {img_file}: {error}")
                        continue
    ...
    if not results:
        st.warning("No processable images found in the directory")
